...
f = open(os.path.join('ParsedData', 'mentions_edges.txt'),'r')
for i in f:
    i = i.rsplit("\n")[0]
    i = i.rsplit("\t")
...
for i in users:
    ...
    x = [i[1],i[0]]
    if str(x) in busers:
        if str([i[1],i[0]]) not in visited and str([i[0],i[1]]) not in visited:
            sortedL = sorted([dicusers[i[0]],i[1], dicusers[i[1]]])
            edges.append(zip([sortedL[0]],[sortedL[1]])[0])
            visited.add(str([i[1],i[0]]))
            visited.add(str([i[0],i[1]]))
...
filepath = open(os.path.join('ParsedData', 'mentions_clusters.csv'),'w')
